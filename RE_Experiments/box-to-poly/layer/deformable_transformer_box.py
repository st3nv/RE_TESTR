...
# ------------------------------------------------------------------------
# This is modified from TESTR to study the box-to-ploy module
# the guidance phi(x) is replaced by a learnable embedding
...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed, text_embed, text_pos_embed, text_mask=None):
        ...
        # torch.Size([1, 100, 4])
        # query_pos SHAPE AFTER PHI
        # torch.Size([1, 100, 256])
        # query_embed BEFORE SHAPE
        # torch.Size([100, 16, 256])
        # query_embed SHAPE
        # torch.Size([1, 100, 16, 256])
        # query_pos SHAPE AFTER REPEAT
        # torch.Size([1, 100, 16, 256])
        # ----------END ---------
        
        
        # ------------------- working here -------------------
        query_pos_exp = self.phi_func_embed.weight.unsqueeze(0).expand(bs, -1, -1)
        query_pos_exp = query_pos_exp[:, :, None, :].repeat(1, 1, query_embed.shape[2], 1)
        # ------------------- working here -------------------        

        # decoder
        hs, hs_text, inter_references = self.decoder(
            query_embed, text_embed, reference_points, memory, spatial_shapes, 
            level_start_index, valid_ratios, query_pos_exp, text_pos_embed, mask_flatten, text_mask
        )
