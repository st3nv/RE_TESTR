...
# ------------------------------------------------------------------------
# This is modified from TESTR to study the factorized attention mechanism
# Both branch's factorized SA are changed to basic SA
...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        # TODO
        self.phi_func_embed = nn.Embedding(self.num_proposals, self.d_model)
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed, text_embed, text_pos_embed, text_mask=None):
        ...
        # output_memory becomes enc_outputs_class which is to find the topk proposals
        ...
        # topk_coords_unact is imoprtant
...
class DeformableCompositeTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
        ...
        self.attn_normal = nn.MultiheadAttention(d_model, n_heads, dropout=dropout)
        self.dropout_normal = nn.Dropout(dropout)
        self.norm_normal = nn.LayerNorm(d_model)
        ...
        self.attn_normal_text = nn.MultiheadAttention(d_model, n_heads, dropout=dropout)
        self.dropout_normal_text = nn.Dropout(dropout)
        self.norm_normal_text = nn.LayerNorm(d_model)
    ...
    def forward(self, tgt, query_pos, tgt_text, query_pos_text, reference_points, src, src_spatial_shapes, level_start_index, src_padding_mask=None, text_padding_mask=None):
        ...
        # ! WORKING HERE -------------------------------------------------------
        # With no factorized attention, the text branch is just a copy of the location branch
        q = k = self.with_pos_embed(tgt, query_pos)
        tgt2 = self.attn_normal(
            q.flatten(0, 2),
            k.flatten(0, 2),
            tgt.flatten(0, 2)
        )[0].reshape(q.shape)
        tgt_normal = tgt + self.dropout_normal(tgt2)
        tgt_normal = self.norm_normal(tgt_normal)
        ...
        # q,k:  torch.Size([1, 100, 16, 256])
        # q.flat.trans:  torch.Size([16, 100, 256])
        # att_intra:  torch.Size([16, 100, 256])
        # test2 [1]:  torch.Size([100, 16, 16])
        ...
        # cross attention
        reference_points_loc = reference_points[:, :, None, :, :].repeat(1, 1, tgt_normal.shape[2], 1, 1)
        tgt2 = self.attn_cross(self.with_pos_embed(tgt_normal, query_pos).flatten(1, 2),
                               reference_points_loc.flatten(1, 2),
                               src, src_spatial_shapes, level_start_index, src_padding_mask).reshape(tgt_normal.shape)
        tgt_normal = tgt_normal + self.dropout_cross(tgt2)
        tgt = self.norm_cross(tgt_normal)


        # ! WORKING HERE -------------------------------------------------------
        # Text branch
        # With no factorized attention
        q_text = k_text = self.with_pos_embed(tgt_text, query_pos_text)
        tgt2_text = self.attn_normal_text(
            q_text.flatten(0, 2),
            k_text.flatten(0, 2),
            tgt_text.flatten(0, 2),
            text_padding_mask.flatten(0, 2) if text_padding_mask is not None else None,
        )[0].reshape(q_text.shape)
        tgt_text_normal = tgt_text + self.dropout_normal_text(tgt2_text)
        tgt_text_normal = self.norm_normal_text(tgt_text_normal)
        ...
        reference_points_text = reference_points[:, :, None, :, :].repeat(1, 1, tgt_text_normal.shape[2], 1, 1)
        tgt2_text_cm = self.attn_cross_text(self.with_pos_embed(tgt_text_normal, query_pos_text).flatten(1, 2),
                                            reference_points_text.flatten(1, 2),
                                            src, src_spatial_shapes, level_start_index, src_padding_mask).reshape(tgt_text_normal.shape)
        
        tgt_text_normal = tgt_text_normal + self.dropout_cross_text(tgt2_text_cm)
        tgt_text = self.norm_cross_text(tgt_text_normal)
